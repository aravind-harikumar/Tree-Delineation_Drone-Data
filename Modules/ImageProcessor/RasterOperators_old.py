...
def StackReprojCropPhoto(bandlist, in_file_name, OutFileName, rs_data_type, data_type, AreaShpFile):    
...
def ReprojectImageCoreg(input_image, outputimage, points, dst_crs):
    ...
    with rasterio.open(input_image) as src:
        ...
        with rasterio.open(outputimage, 'w', **kwargs) as dst:
            with alive_bar(src.count) as bar:
...
def ReprojectImage(input_image, outputimage, dst_crs, Nodataval=0):
    # dst_crs = 'EPSG:32619'
...
def CropImage_direct(InputImage, ShpFile, OutFileName, NoDataVal):
    with fiona.open(ShpFile, 'r') as shapefile:
        ShapeMask = [feature["geometry"] for feature in shapefile]
        with rasterio.open(InputImage) as ImageObj:
            out_image, out_transform = rasterio.mask.mask(ImageObj, ShapeMask, crop=True, filled=True, nodata = 0)
            out_meta = ImageObj.meta.copy()
            out_meta.update({"driver": "GTiff",
                    "height": out_image.shape[1],
                    "width": out_image.shape[2],
...
def AlignRaster(inputfile, referencefile, outputfile, out_ndsm_org_res_file, outputfileOrtho):
    ...
    for i in range(1,reference.RasterCount+1):
        dst_ortho.GetRasterBand(i).SetNoDataValue(0)
