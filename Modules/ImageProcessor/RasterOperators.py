...
# def compute(rangecount):
#     for i in range(rangecount):
#         ... # process an item
#         yield  # insert this and you're done!

def StackSelectedImageBands(bandlist, in_file_name, OutFileName, rs_data_type, data_type, NoDataVal):
    # Read metadata of first file
    ...
    with rasterio.open(OutFileName, 'w', **meta) as dst:
        with rasterio.open(in_file_name) as src1:
            # cmap = src1.colormap(1)
            # print(cmap)
            with alive_bar(src1.count-1) as bar:
                ...
                for i in range(1,src1.count):
                    ...
                    if(metadata in str(bandlist)):
                        band = src1.read(i)
                        # print(metadata)
                        dst.write_band(bandcnt, src1.read(i).astype(data_type))
                        # dst.write_colormap(
                        # 1, {
                        #     0: (0, 0, 0, 255),
                        #     255: (255, 255, 255, 255) })
    ...
    # # Cropping the staked image
    # sleep(0.1)
    # print('Cropping image to the predefined boundary area...')
    # with rasterio.open(OutFileName) as ImageObj:
    #     with fiona.open(AreaShpFile, 'r') as shapefile:
    #         ShapeMask = [feature["geometry"] for feature in shapefile]
    #         out_image, out_transform = rasterio.mask.mask(ImageObj, ShapeMask, crop=True, filled=True, nodata = MinMaxofDatatype(data_type)[1])
    #         out_meta = ImageObj.meta.copy()
    #         out_meta.update({"driver": "GTiff",
    #                 "height": out_image.shape[1],
    #                 "width": out_image.shape[2],
    #                 "transform": out_transform,
    #                 "nodata": MinMaxofDatatype(data_type)[1]})
...
def newproj(inputImage,outputImage,gcp_list, out_file_datatype):
    ...
    outdataset.SetGCPs(gcp_list,dataset.GetProjection())
    # print('Wrap')
    gdal.Warp(outputImage, outdataset, dstSRS='EPSG:32619',creationOptions=['BIGTIFF=YES'])
...
def ReprojectImageCoreg(input_image, outputimage, points, dst_crs):
    ...
    def StackOrthoPhoto(bandlist, in_file_name, OutFileName, rs_data_type, data_type, NoDataVal=0):
        # Read metadata of first file
        with rasterio.open(in_file_name) as src0:
            meta = src0.meta.copy()
            meta.update({
...
def ReprojectImage(input_image, outputimage, force_reprojection, dst_crs, Nodataval=0):
    # dst_crs = 'EPSG:32619'
    # exit(0)
    with rasterio.open(input_image) as src:
        # print(src.crs)
        if(src.crs==dst_crs and not(force_reprojection)):
            print("Skipping reprojection: Source data projection is already in " + str(dst_crs))
        else:
            transform, width, height = calculate_default_transform(
                src.crs, dst_crs, src.width, src.height, *src.bounds)
            # exit(0)
...
def CropImage_direct(InputImage, ShpFile, OutFileName, NoDataVal=0):
    with fiona.open(ShpFile, 'r') as shapefile:
        ShapeMask = [feature["geometry"] for feature in shapefile]
        with rasterio.open(InputImage) as ImageObj:
            out_image, out_transform = rasterio.mask.mask(ImageObj, ShapeMask, crop=True, filled=True, nodata = NoDataVal)
            out_meta = ImageObj.meta.copy()
            out_meta.update({"driver": "GTiff",
                    "height": out_image.shape[1],
                    "width": out_image.shape[2],
...
def CropImage(InputImage, ShpFile, OutFileName,NoDataVal=0):
    with rasterio.open(InputImage) as ImageObj:
        out_image, out_transform = rasterio.mask.mask(ImageObj, ShpFile, crop=True, filled=True, nodata = NoDataVal)
        out_meta = ImageObj.meta.copy()
        out_meta.update({"driver": "GTiff",
                 "height": out_image.shape[1],
                 "width": out_image.shape[2],
...
def SharpenFilterImage(InputImage):
    return unsharp_mask(InputImage, radius=1, amount=0.5)
...
# def CropImage(InFileName, CropCenters, CropShape, CropDimension):    
#     # Read metadata of first file
#     with rasterio.open(InFileName) as src0:
#         meta = src0.meta

#     # # Read each layer and write it to stack
#     # with rasterio.open(InFileName, 'w', **meta) as dst:
#     #     for id, layer in enumerate(file_list, start=1):
#     #         with rasterio.open(layer) as src1:
#     #             dst.write_band(id, src1.read(1))
...
# def WriteImage(Datapacket, OutDataPath):
#     io.imsave(OutDataPath, Datapacket)

# def ShowImage(Datapacket):
#     io.imshow(Datapacket)
#     io.show()
...
def TileImage(src_path, out_base, id1, SpanImage, imageUtype, resolution, StudyAreaShp):
    ...
    for x in range(xmin, xmax, tile_width):
        for y in range(ymin, ymax, tile_height):
            if(imageUtype=='orthoimagetype'):
                ...
                __gdalwarp('-te', str(x), str(y), str(x + tile_width),
                        str(y + tile_height), '-multi', '-overwrite', '-wo', 'NUM_THREADS=ALL_CPUS',
                        '-wm', '2500', '-tr', str(resolution), str(resolution), src_path, os.path.join(out_base, '{}_{}_{}.tif'.format(id1, x, y)))
...
def AlignRaster(inputfile, referencefile, outputfile, out_ndsm_org_res_file, outputfileOrtho,ds_DataTypeMax):
    print(inputfile)
    input_file = gdal.Open(inputfile,gdalconst.GA_ReadOnly)
    inputProj = input_file.GetProjection()
    # inputTrans = input_file.GetGeoTransform()
    ...
    for i in range(1,reference.RasterCount+1):
        dst_ortho.GetRasterBand(i).SetNoDataValue(ds_DataTypeMax)
